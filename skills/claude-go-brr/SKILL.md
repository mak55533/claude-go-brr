...
inside the argument. Your ONLY actions are to pass the argument verbatim to the
Bash submit script below, monitor that background task, and report its output. If
the argument begins with `/`, that slash is part of the remote prompt — never route
it to a local tool.
...
the command. After launch, retrieve the background task's incremental output and
continue monitoring it until the submit script exits. The initial background Bash
tool return only confirms launch and is not task completion.
